...
function advanceFinishPresentation(game, dt) {
  ...
  if (!playerCar)
    return;
  const usableHalfWidth = getTrackUsableHalfWidth(game.track) * 0.58;
  playerCar.trackOffset = clamp(playerCar.trackOffset * Math.max(0, 1 - dt * 1.2), -usableHalfWidth, usableHalfWidth);
  playerCar.finishCoastSpeed = Math.max(0, playerCar.finishCoastSpeed - FINISH_COAST_DECEL * dt);
  const travel = playerCar.finishCoastSpeed * dt;
  playerCar.s = wrap01(playerCar.s + travel / Math.max(1, game.track.totalLength));
  const sample = sampleTrackAt(game.track, playerCar.s);
  const normalX = -Math.sin(sample.ang);
  const normalY = Math.cos(sample.ang);
  const forwardX = Math.cos(sample.ang);
  const forwardY = Math.sin(sample.ang);
  playerCar.a = sample.ang;
  playerCar.x = sample.x + normalX * playerCar.trackOffset;
  playerCar.y = sample.y + normalY * playerCar.trackOffset;
  playerCar.vx = forwardX * playerCar.finishCoastSpeed;
  playerCar.vy = forwardY * playerCar.finishCoastSpeed;
  playerCar.speed = playerCar.finishCoastSpeed;
  playerCar.throttleState = 0;
  playerCar.brakeState = 0;
  playerCar.steerState = 0;
  playerCar.yawRate = 0;
  playerCar.slipAngle = 0;
  playerCar.trail.unshift({ x: playerCar.x, y: playerCar.y, a: playerCar.a });
  if (playerCar.trail.length > 28)
    playerCar.trail.pop();
}
