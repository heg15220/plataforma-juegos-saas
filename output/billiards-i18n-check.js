...
function createPlayers(difficultyKey, locale) {
  return [
    { name: locale === "es" ? "Tu" : "You", type: "human", group: null, racksWon: 0, foulsInRow: 0 },
    { name: `${locale === "es" ? "IA" : "AI"} ${difficultyLabel(difficultyKey, locale)}`, type: "ai", group: null, racksWon: 0, foulsInRow: 0 }
  ];
}
function createRuntimeState(modeKey = "eight-ball", difficultyKey = "club", locale = resolveLocale()) {
  ...
  return {
    ...
    difficultyKey,
    raceTo: 3,
    players: createPlayers(difficultyKey, locale),
...
function drawTable(ctx, state, preview, placementGhost) {
  ...
  if (preview) {
    const aiGuide = state.phase === "ai-thinking" && state.currentPlayer === PLAYER_AI;
...
function createRuntime({ canvas, onSnapshot, onFullscreenRequest, isTableRotated = () => false, locale = resolveLocale() }) {
  ...
  const runtime = {
    canvas,
    ctx,
    state: createRuntimeState("eight-ball", "club", locale),
    ...
    resetToMenu(modeKey = this.state.modeKey, difficultyKey = this.state.difficultyKey) {
      this.state = createRuntimeState(modeKey, difficultyKey, this.state.locale ?? locale);
      this.refresh();
    },
    startMatch() {
      const nextState = createRuntimeState(this.state.modeKey, this.state.difficultyKey, this.state.locale ?? locale);
...
function BilliardsClubGame() {
  ...
  const modeObjective = modeSummary(snapshot.variant, locale);
