...
for (const view of views) {
  ...
  await page.screenshot({ path: path.join(outDir, `domino-${view.name}-top.png`) });
  ...
  await page.screenshot({ path: path.join(outDir, `domino-${view.name}-bottom.png`) });
  ...
  fs.writeFileSync(path.join(outDir, `domino-${view.name}-probe.json`), JSON.stringify(probe, null, 2));
  ...
  fs.writeFileSync(path.join(outDir, `domino-${view.name}-state.json`), String(renderState ?? "null"));

  if (errors.length) {
    fs.writeFileSync(path.join(outDir, `domino-${view.name}-errors.json`), JSON.stringify(errors, null, 2));
  }
