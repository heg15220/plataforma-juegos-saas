  ...
  const handle = await page.evaluateHandle(() => {
    ...
    for (const canvas of document.querySelectorAll("canvas")) {
      ...
      const visible =
        ...
        style.opacity !== "0";
      const backingArea = (canvas.width || 0) * (canvas.height || 0);
      const viewportArea = rect.width * rect.height;
      const score = (visible ? 1_000_000_000 : 0) + Math.max(backingArea, viewportArea);
      if (score > bestScore) {
        bestScore = score;
        best = canvas;
      }
...
async function main() {
  ...
  try {
    ...
    // Continue even if a large canvas is not detected yet.
  }
